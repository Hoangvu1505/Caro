...
void InitGameAudio() {
    ...
    if (!_BGM_ON) PauseMusicStream(gMenuMusic);
...
void UpdateGameAudio() {
    ...
    static bool previousBgmState = true;
    if (_BGM_ON && !previousBgmState) {
        if (gCurrentMusic == 0) ResumeMusicStream(gMenuMusic);
        else if (gCurrentMusic == 1) ResumeMusicStream(gGameMusic);
        previousBgmState = true;
    }
    else if (!_BGM_ON && previousBgmState) {
        if (gCurrentMusic == 0) PauseMusicStream(gMenuMusic);
        else if (gCurrentMusic == 1) PauseMusicStream(gGameMusic);
        previousBgmState = false;
    }

    // Chỉ Update luồng nhạc nếu đang BẬT
    if (_BGM_ON) {
...
void PlayNavigateSfx() {
    if (gAudioInitialized && _SFX_ON) PlaySound(gNavigateSfx);
}

void PlaySelectSfx() {
    if (gAudioInitialized && _SFX_ON) PlaySound(gSelectSfx);
}

void PlayBoardMoveSfx() {
    if (gAudioInitialized && _SFX_ON) PlaySound(gMoveSfx);
}

void PlayPlaceSfx() {
    if (gAudioInitialized && _SFX_ON) PlaySound(gPlaceSfx);
}
